...
describe Answer do
  ...
  describe '.update_through_api' do
    let(:perform) { described_class.update_through_api(params) }

    context 'when params are correct' do
      let!(:params) { {id: answer.id, result: '3', secret: answer.secret} }
    ...
    context 'when answer is correct but params are not correct because' do
      ...
      context 'secret is wrong' do
        let!(:params) { {id: answer.id, result: '3', secret: 'error'} }
      ...
      context 'id is wrong' do
        let!(:params) { {id: 0, result: '3', secret: answer.secret} }
      ...
      context 'result is wrong' do
        let!(:params) { {id: answer.id, result: '5', secret: answer.secret} }
